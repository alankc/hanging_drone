...
class LandingPipeline:
    ...
    def __init__(self, ed:EasyDrone, s:Stereo, v:Vision, yd:YOLODetector, bbox, frame, cx, cy, odom_file, ty:float, drone_hook_center:float) -> None:
        self.__ed = ed
        self.__s = s
        self.__v = v
        self.__yd = yd
    ...
    def state_yolo(self):
        ...
        if not ((pt1 is None) and (pt2 is None)):
            ...
            if (abs(error_cy) < 10) and (abs(error_cx) < 10) and (abs(ctrl_s_yaw) < 0.1) and (abs(ctrl_s_throttle) < 0.1):
                ...
                #Initialize tracker
                bbox = (pt1[0], pt1[1], pt2[0] - pt1[0], pt2[1] - pt1[1]) #(xmin,ymin,boxwidth,boxheight)
